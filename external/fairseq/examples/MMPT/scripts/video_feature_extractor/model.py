...
def get_model(args):
    ...
    if args.type == '2d':
        ...
        repo_path = dirname(dirname(dirname(dirname(dirname(dirname(dirname(__file__)))))))
        print(repo_path)
        model_dir = join(repo_path, 'external/requirements/fairseq')
        model = S3D(join(model_dir, 'pretrained_models/s3d_dict.npy'), 512)
        model.load_state_dict(th.load(join(model_dir, 'pretrained_models/s3d_howto100m.pth')))
        model = model.cuda() if th.cuda.is_available() else model
