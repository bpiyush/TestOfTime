...
# iterator = tqdm(range(len(dataset)), desc="Checking existing files")
# indices = [i for i in iterator if not os.path.exists(dataset[i]["output"])]

args.end_idx = len(dataset) if args.end_idx == -1 else args.end_idx
dataset = Subset(dataset, range(args.start_idx, args.end_idx))
print(">>> Running on {} videos: {} to {}".format(len(dataset), args.start_idx, args.end_idx))

# # check if the feature already exists
# indices = [i for i in tqdm(range(len(dataset)), desc="Checking existance") if not os.path.exists(dataset[i]["output"])]
            ...
            if len(video.shape) == 4:
                ...
                if args.type == 'vmz':
                    ...
                    if cuda:
                        ...
                        features = th.FloatTensor(n_chunk, 512).fill_(0)
                elif args.type == 's3d':
                    if cuda:
                        features = th.cuda.FloatTensor(n_chunk, 512).fill_(0)
                    else:
                    ...
                    if cuda:
                        ...
                        features = th.LongTensor(n_chunk, 1024).fill_(0)
                else:
                    if cuda:
                ...
                for i in range(n_iter):
                    ...
                    if cuda:
                        ...
                        video_batch = video[min_ind:max_ind:factor]
                    if args.type == '2d':
