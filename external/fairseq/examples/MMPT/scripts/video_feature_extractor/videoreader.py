    ...
    def __init__(
        ...
        **kwargs
        ...
        if video_dict is not None:
            ...
            indices = list(map(lambda x: not exists(x), video_dict["feature_path"]))
            print(">>> Filtering only videos that do not have features.")
            print(">>> Found {} videos that do not have features.".format(sum(indices)))
            video_dict = {k: [v[i] for i, boolean in enumerate(indices) if boolean] for k, v in video_dict.items()}
