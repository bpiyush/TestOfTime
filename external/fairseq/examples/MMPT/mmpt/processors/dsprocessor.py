...
class TEMPOVideoProcessor(VideoProcessor):
    """
    ...
    def __call__(self, video_fn):
        if isinstance(video_fn, tuple):
            assert len(video_fn) == 6, "video_fn should be a tuple of 6"\
                " elements: (video_fn, tau, x_start, x_end, y_start, y_end). The times are in seconds."
            video_fn, tau, x_start_time, x_end_time, y_start_time, y_end_time = video_fn
            x_start_time, x_end_time = int(x_start_time), int(x_end_time)
            y_start_time, y_end_time = int(y_start_time), int(y_end_time)
        else:
            tau = None
        ...
        if tau == "before":
            feat = np.concatenate([feat[x_start_time:x_end_time], feat[y_start_time:y_end_time]])
        elif tau == "after":
            feat = np.concatenate([feat[y_start_time:y_end_time], feat[x_start_time:x_end_time]])
        elif tau == None:
            pass

        # feat = feat[start_time:end_time]
        return feat


class TEMPOMetaProcessor(MetaProcessor):
    """TEMPO dataset.
    """
    ...
    def __init__(self, config):
        ...
        self.use_time_bdry = config.get("use_time_bdry", True)
    ...
    def __getitem__(self, idx):
        ...
        x_start_time, x_end_time = eval(self.data["event_X_time_in_secs"].values[idx])
        y_start_time, y_end_time = eval(self.data["event_Y_time_in_secs"].values[idx])
        tau = self.data["temporal_preposition"].values[idx]
        # end_time = self.data["end_time"].values[idx]
        
        if self.use_time_bdry:
            vid_info = (vid, tau, x_start_time, x_end_time, y_start_time, y_end_time)
        else:
            vid_info = vid
        if "sentence" in self.data:  # for testing.
            ...
            sentence = vid
        return vid_info, sentence


class TEMPOTextProcessor(TextProcessor):
    """TEMPO dataset.
...
class YoucookMetaProcessor(MetaProcessor):
    ...
    def __init__(self, config):
        ...
        with open(self._get_split_path(config), "rb") as fd:
            ...
            all_valid_video_ids = set(
                ...
                ["/".join(os.path.splitext(fn)[0].split("/")[-2:]) for fn in glob(vfeat_dir + "*/*.npy")]
            )
            recs = []
            video_ids = set()
            valid_video_ids = set()
            for rec in data:  # filter videos not available.
                ...
                flags = [x.endswith(video_id) for x in all_valid_video_ids]
                if sum(flags) == 1:
                    idx = flags.index(True)
                    video_id = list(all_valid_video_ids)[idx]
                    valid_video_ids.add(video_id)
                    rec["id"] = video_id + "_" + rec["id"][udl_idx + 1:]
                    recs.append(rec)
    ...
    def __getitem__(self, idx):
        def _get_video_and_caption(rec):
            ...
            clip = self.youcook_annotation[video_id.split("/")[-1]]["annotations"][clip_id]
